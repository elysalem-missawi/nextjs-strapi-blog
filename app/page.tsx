...
const Stats = () => {
  ...
  return (
    <section ref={ref} className="py-16 bg-gradient-to-b from-sky-50/30 to-transparent dark:from-slate-900/20">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto text-center">
        {[
          ...
          >
            ...
            <div className="text-sm font-medium text-slate-600 dark:text-slate-400 mt-1">{stat.label}</div>
          </motion.div>
        ))}
      </div>
    </section>
...
const SkillCircle = ({ name, category }) => {
  // نسبة وهمية بناءً على التصنيف (يمكن جعلها ديناميكية)
  const percentage = category === "لغات برمجة" ? 90 : category === "أطر عمل" ? 85 : category === "أدوات" ? 75 : 80;
  ...
  return (
    ...
    >
      <div className="relative w-24 h-24">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          ...
          <circle
            ...
            strokeDashoffset={2 * Math.PI * 42 * (1 - (isVisible ? percentage : 0) / 100)}
      ...
      <span className="mt-2 font-bold text-slate-800 dark:text-slate-100 text-sm text-center">{name}</span>
  ...
  return (
    <main className="min-h-screen p-6 md:p-12 max-w-6xl mx-auto space-y-20" dir="rtl">
      ...
      >
        ...
        >
          {/* النصوص */}
          <motion.div variants={{ hidden: { opacity: 0, x: -30 }, visible: { opacity: 1, x: 0 } }} className="space-y-4 max-w-xl">
          ...
          {/* الصورة الشخصية مع تأثير parallax بسيط */}
          <motion.div
      ...
      <section id="skills" className="space-y-8">
        <div className="flex items-center gap-3 border-b border-sky-100 dark:border-slate-800 pb-4">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white">المهارات التقنية</h2>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
          {skillsData.map((skill) => (
            <SkillCircle key={skill.name} name={skill.name} category={skill.category} />
          ))}
        </div>
      </section>
      ...
      <section id="projects" className="space-y-8">
        <div className="flex items-center gap-3 border-b border-sky-100 dark:border-slate-800 pb-4">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white">المشاريع المتميزة</h2>
        </div>
      ...
      <section id="contact" className="space-y-8 py-10">
        <div className="flex items-center gap-3 border-b border-sky-100 dark:border-slate-800 pb-4">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white">تواصل معي</h2>
        </div>
